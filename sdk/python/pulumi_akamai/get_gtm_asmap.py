# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from . import _utilities
from . import outputs
from ._inputs import *

__all__ = [
    'GetGtmAsmapResult',
    'AwaitableGetGtmAsmapResult',
    'get_gtm_asmap',
    'get_gtm_asmap_output',
]

@pulumi.output_type
class GetGtmAsmapResult:
    """
    A collection of values returned by getGtmAsmap.
    """
    def __init__(__self__, assignments=None, default_datacenter=None, domain=None, links=None, map_name=None):
        if assignments and not isinstance(assignments, list):
            raise TypeError("Expected argument 'assignments' to be a list")
        pulumi.set(__self__, "assignments", assignments)
        if default_datacenter and not isinstance(default_datacenter, dict):
            raise TypeError("Expected argument 'default_datacenter' to be a dict")
        pulumi.set(__self__, "default_datacenter", default_datacenter)
        if domain and not isinstance(domain, str):
            raise TypeError("Expected argument 'domain' to be a str")
        pulumi.set(__self__, "domain", domain)
        if links and not isinstance(links, list):
            raise TypeError("Expected argument 'links' to be a list")
        pulumi.set(__self__, "links", links)
        if map_name and not isinstance(map_name, str):
            raise TypeError("Expected argument 'map_name' to be a str")
        pulumi.set(__self__, "map_name", map_name)

    @_builtins.property
    @pulumi.getter
    def assignments(self) -> Optional[Sequence['outputs.GetGtmAsmapAssignmentResult']]:
        return pulumi.get(self, "assignments")

    @_builtins.property
    @pulumi.getter(name="defaultDatacenter")
    def default_datacenter(self) -> Optional['outputs.GetGtmAsmapDefaultDatacenterResult']:
        return pulumi.get(self, "default_datacenter")

    @_builtins.property
    @pulumi.getter
    def domain(self) -> _builtins.str:
        return pulumi.get(self, "domain")

    @_builtins.property
    @pulumi.getter
    def links(self) -> Optional[Sequence['outputs.GetGtmAsmapLinkResult']]:
        return pulumi.get(self, "links")

    @_builtins.property
    @pulumi.getter(name="mapName")
    def map_name(self) -> _builtins.str:
        return pulumi.get(self, "map_name")


class AwaitableGetGtmAsmapResult(GetGtmAsmapResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetGtmAsmapResult(
            assignments=self.assignments,
            default_datacenter=self.default_datacenter,
            domain=self.domain,
            links=self.links,
            map_name=self.map_name)


def get_gtm_asmap(assignments: Optional[Sequence[Union['GetGtmAsmapAssignmentArgs', 'GetGtmAsmapAssignmentArgsDict', 'outputs.GetGtmAsmapAssignmentResult']]] = None,
                  default_datacenter: Optional[Union['GetGtmAsmapDefaultDatacenterArgs', 'GetGtmAsmapDefaultDatacenterArgsDict', 'outputs.GetGtmAsmapDefaultDatacenterResult']] = None,
                  domain: Optional[_builtins.str] = None,
                  links: Optional[Sequence[Union['GetGtmAsmapLinkArgs', 'GetGtmAsmapLinkArgsDict', 'outputs.GetGtmAsmapLinkResult']]] = None,
                  map_name: Optional[_builtins.str] = None,
                  opts: Optional[pulumi.InvokeOptions] = None) -> AwaitableGetGtmAsmapResult:
    """
    Use this data source to access information about an existing resource.
    """
    __args__ = dict()
    __args__['assignments'] = assignments
    __args__['defaultDatacenter'] = default_datacenter
    __args__['domain'] = domain
    __args__['links'] = links
    __args__['mapName'] = map_name
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke('akamai:index/getGtmAsmap:getGtmAsmap', __args__, opts=opts, typ=GetGtmAsmapResult).value

    return AwaitableGetGtmAsmapResult(
        assignments=pulumi.get(__ret__, 'assignments'),
        default_datacenter=pulumi.get(__ret__, 'default_datacenter'),
        domain=pulumi.get(__ret__, 'domain'),
        links=pulumi.get(__ret__, 'links'),
        map_name=pulumi.get(__ret__, 'map_name'))
def get_gtm_asmap_output(assignments: pulumi.Input[Optional[Optional[Sequence[Union['GetGtmAsmapAssignmentArgs', 'GetGtmAsmapAssignmentArgsDict', 'outputs.GetGtmAsmapAssignmentResult']]]]] = None,
                         default_datacenter: pulumi.Input[Optional[Optional[Union['GetGtmAsmapDefaultDatacenterArgs', 'GetGtmAsmapDefaultDatacenterArgsDict', 'outputs.GetGtmAsmapDefaultDatacenterResult']]]] = None,
                         domain: pulumi.Input[Optional[_builtins.str]] = None,
                         links: pulumi.Input[Optional[Optional[Sequence[Union['GetGtmAsmapLinkArgs', 'GetGtmAsmapLinkArgsDict', 'outputs.GetGtmAsmapLinkResult']]]]] = None,
                         map_name: pulumi.Input[Optional[_builtins.str]] = None,
                         opts: Optional[Union[pulumi.InvokeOptions, pulumi.InvokeOutputOptions]] = None) -> pulumi.Output[GetGtmAsmapResult]:
    """
    Use this data source to access information about an existing resource.
    """
    __args__ = dict()
    __args__['assignments'] = assignments
    __args__['defaultDatacenter'] = default_datacenter
    __args__['domain'] = domain
    __args__['links'] = links
    __args__['mapName'] = map_name
    opts = pulumi.InvokeOutputOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke_output('akamai:index/getGtmAsmap:getGtmAsmap', __args__, opts=opts, typ=GetGtmAsmapResult)
    return __ret__.apply(lambda __response__: GetGtmAsmapResult(
        assignments=pulumi.get(__response__, 'assignments'),
        default_datacenter=pulumi.get(__response__, 'default_datacenter'),
        domain=pulumi.get(__response__, 'domain'),
        links=pulumi.get(__response__, 'links'),
        map_name=pulumi.get(__response__, 'map_name')))
