# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins
import copy
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from . import _utilities

__all__ = ['BotmanServeAlternateActionArgs', 'BotmanServeAlternateAction']

@pulumi.input_type
class BotmanServeAlternateActionArgs:
    def __init__(__self__, *,
                 config_id: pulumi.Input[builtins.int],
                 serve_alternate_action: pulumi.Input[builtins.str]):
        """
        The set of arguments for constructing a BotmanServeAlternateAction resource.
        """
        pulumi.set(__self__, "config_id", config_id)
        pulumi.set(__self__, "serve_alternate_action", serve_alternate_action)

    @property
    @pulumi.getter(name="configId")
    def config_id(self) -> pulumi.Input[builtins.int]:
        return pulumi.get(self, "config_id")

    @config_id.setter
    def config_id(self, value: pulumi.Input[builtins.int]):
        pulumi.set(self, "config_id", value)

    @property
    @pulumi.getter(name="serveAlternateAction")
    def serve_alternate_action(self) -> pulumi.Input[builtins.str]:
        return pulumi.get(self, "serve_alternate_action")

    @serve_alternate_action.setter
    def serve_alternate_action(self, value: pulumi.Input[builtins.str]):
        pulumi.set(self, "serve_alternate_action", value)


@pulumi.input_type
class _BotmanServeAlternateActionState:
    def __init__(__self__, *,
                 action_id: Optional[pulumi.Input[builtins.str]] = None,
                 config_id: Optional[pulumi.Input[builtins.int]] = None,
                 serve_alternate_action: Optional[pulumi.Input[builtins.str]] = None):
        """
        Input properties used for looking up and filtering BotmanServeAlternateAction resources.
        """
        if action_id is not None:
            pulumi.set(__self__, "action_id", action_id)
        if config_id is not None:
            pulumi.set(__self__, "config_id", config_id)
        if serve_alternate_action is not None:
            pulumi.set(__self__, "serve_alternate_action", serve_alternate_action)

    @property
    @pulumi.getter(name="actionId")
    def action_id(self) -> Optional[pulumi.Input[builtins.str]]:
        return pulumi.get(self, "action_id")

    @action_id.setter
    def action_id(self, value: Optional[pulumi.Input[builtins.str]]):
        pulumi.set(self, "action_id", value)

    @property
    @pulumi.getter(name="configId")
    def config_id(self) -> Optional[pulumi.Input[builtins.int]]:
        return pulumi.get(self, "config_id")

    @config_id.setter
    def config_id(self, value: Optional[pulumi.Input[builtins.int]]):
        pulumi.set(self, "config_id", value)

    @property
    @pulumi.getter(name="serveAlternateAction")
    def serve_alternate_action(self) -> Optional[pulumi.Input[builtins.str]]:
        return pulumi.get(self, "serve_alternate_action")

    @serve_alternate_action.setter
    def serve_alternate_action(self, value: Optional[pulumi.Input[builtins.str]]):
        pulumi.set(self, "serve_alternate_action", value)


@pulumi.type_token("akamai:index/botmanServeAlternateAction:BotmanServeAlternateAction")
class BotmanServeAlternateAction(pulumi.CustomResource):
    @overload
    def __init__(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 config_id: Optional[pulumi.Input[builtins.int]] = None,
                 serve_alternate_action: Optional[pulumi.Input[builtins.str]] = None,
                 __props__=None):
        """
        Create a BotmanServeAlternateAction resource with the given unique name, props, and options.
        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        ...
    @overload
    def __init__(__self__,
                 resource_name: str,
                 args: BotmanServeAlternateActionArgs,
                 opts: Optional[pulumi.ResourceOptions] = None):
        """
        Create a BotmanServeAlternateAction resource with the given unique name, props, and options.
        :param str resource_name: The name of the resource.
        :param BotmanServeAlternateActionArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        ...
    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(BotmanServeAlternateActionArgs, pulumi.ResourceOptions, *args, **kwargs)
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 config_id: Optional[pulumi.Input[builtins.int]] = None,
                 serve_alternate_action: Optional[pulumi.Input[builtins.str]] = None,
                 __props__=None):
        opts = pulumi.ResourceOptions.merge(_utilities.get_resource_opts_defaults(), opts)
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError('Expected resource options to be a ResourceOptions instance')
        if opts.id is None:
            if __props__ is not None:
                raise TypeError('__props__ is only valid when passed in combination with a valid opts.id to get an existing resource')
            __props__ = BotmanServeAlternateActionArgs.__new__(BotmanServeAlternateActionArgs)

            if config_id is None and not opts.urn:
                raise TypeError("Missing required property 'config_id'")
            __props__.__dict__["config_id"] = config_id
            if serve_alternate_action is None and not opts.urn:
                raise TypeError("Missing required property 'serve_alternate_action'")
            __props__.__dict__["serve_alternate_action"] = serve_alternate_action
            __props__.__dict__["action_id"] = None
        super(BotmanServeAlternateAction, __self__).__init__(
            'akamai:index/botmanServeAlternateAction:BotmanServeAlternateAction',
            resource_name,
            __props__,
            opts)

    @staticmethod
    def get(resource_name: str,
            id: pulumi.Input[str],
            opts: Optional[pulumi.ResourceOptions] = None,
            action_id: Optional[pulumi.Input[builtins.str]] = None,
            config_id: Optional[pulumi.Input[builtins.int]] = None,
            serve_alternate_action: Optional[pulumi.Input[builtins.str]] = None) -> 'BotmanServeAlternateAction':
        """
        Get an existing BotmanServeAlternateAction resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _BotmanServeAlternateActionState.__new__(_BotmanServeAlternateActionState)

        __props__.__dict__["action_id"] = action_id
        __props__.__dict__["config_id"] = config_id
        __props__.__dict__["serve_alternate_action"] = serve_alternate_action
        return BotmanServeAlternateAction(resource_name, opts=opts, __props__=__props__)

    @property
    @pulumi.getter(name="actionId")
    def action_id(self) -> pulumi.Output[builtins.str]:
        return pulumi.get(self, "action_id")

    @property
    @pulumi.getter(name="configId")
    def config_id(self) -> pulumi.Output[builtins.int]:
        return pulumi.get(self, "config_id")

    @property
    @pulumi.getter(name="serveAlternateAction")
    def serve_alternate_action(self) -> pulumi.Output[builtins.str]:
        return pulumi.get(self, "serve_alternate_action")

