// Code generated by the Pulumi Terraform Bridge (tfgen) Tool DO NOT EDIT.
// *** WARNING: Do not edit by hand unless you're certain you know what you are doing! ***

package akamai

import (
	"context"
	"reflect"

	"github.com/pulumi/pulumi-akamai/sdk/v8/go/akamai/internal"
	"github.com/pulumi/pulumi/sdk/v3/go/pulumi"
)

func GetPropertyRulesBuilder(ctx *pulumi.Context, args *GetPropertyRulesBuilderArgs, opts ...pulumi.InvokeOption) (*GetPropertyRulesBuilderResult, error) {
	opts = internal.PkgInvokeDefaultOpts(opts)
	var rv GetPropertyRulesBuilderResult
	err := ctx.Invoke("akamai:index/getPropertyRulesBuilder:getPropertyRulesBuilder", args, &rv, opts...)
	if err != nil {
		return nil, err
	}
	return &rv, nil
}

// A collection of arguments for invoking getPropertyRulesBuilder.
type GetPropertyRulesBuilderArgs struct {
	RulesV20230105 *GetPropertyRulesBuilderRulesV20230105 `pulumi:"rulesV20230105"`
	RulesV20230530 *GetPropertyRulesBuilderRulesV20230530 `pulumi:"rulesV20230530"`
	RulesV20230920 *GetPropertyRulesBuilderRulesV20230920 `pulumi:"rulesV20230920"`
	RulesV20231030 *GetPropertyRulesBuilderRulesV20231030 `pulumi:"rulesV20231030"`
	RulesV20240109 *GetPropertyRulesBuilderRulesV20240109 `pulumi:"rulesV20240109"`
	RulesV20240212 *GetPropertyRulesBuilderRulesV20240212 `pulumi:"rulesV20240212"`
	RulesV20240531 *GetPropertyRulesBuilderRulesV20240531 `pulumi:"rulesV20240531"`
	RulesV20240813 *GetPropertyRulesBuilderRulesV20240813 `pulumi:"rulesV20240813"`
	RulesV20241021 *GetPropertyRulesBuilderRulesV20241021 `pulumi:"rulesV20241021"`
	RulesV20250113 *GetPropertyRulesBuilderRulesV20250113 `pulumi:"rulesV20250113"`
	RulesV20250218 *GetPropertyRulesBuilderRulesV20250218 `pulumi:"rulesV20250218"`
}

// A collection of values returned by getPropertyRulesBuilder.
type GetPropertyRulesBuilderResult struct {
	// The provider-assigned unique ID for this managed resource.
	Id             string                                 `pulumi:"id"`
	Json           string                                 `pulumi:"json"`
	RuleFormat     string                                 `pulumi:"ruleFormat"`
	RulesV20230105 *GetPropertyRulesBuilderRulesV20230105 `pulumi:"rulesV20230105"`
	RulesV20230530 *GetPropertyRulesBuilderRulesV20230530 `pulumi:"rulesV20230530"`
	RulesV20230920 *GetPropertyRulesBuilderRulesV20230920 `pulumi:"rulesV20230920"`
	RulesV20231030 *GetPropertyRulesBuilderRulesV20231030 `pulumi:"rulesV20231030"`
	RulesV20240109 *GetPropertyRulesBuilderRulesV20240109 `pulumi:"rulesV20240109"`
	RulesV20240212 *GetPropertyRulesBuilderRulesV20240212 `pulumi:"rulesV20240212"`
	RulesV20240531 *GetPropertyRulesBuilderRulesV20240531 `pulumi:"rulesV20240531"`
	RulesV20240813 *GetPropertyRulesBuilderRulesV20240813 `pulumi:"rulesV20240813"`
	RulesV20241021 *GetPropertyRulesBuilderRulesV20241021 `pulumi:"rulesV20241021"`
	RulesV20250113 *GetPropertyRulesBuilderRulesV20250113 `pulumi:"rulesV20250113"`
	RulesV20250218 *GetPropertyRulesBuilderRulesV20250218 `pulumi:"rulesV20250218"`
}

func GetPropertyRulesBuilderOutput(ctx *pulumi.Context, args GetPropertyRulesBuilderOutputArgs, opts ...pulumi.InvokeOption) GetPropertyRulesBuilderResultOutput {
	return pulumi.ToOutputWithContext(ctx.Context(), args).
		ApplyT(func(v interface{}) (GetPropertyRulesBuilderResultOutput, error) {
			args := v.(GetPropertyRulesBuilderArgs)
			options := pulumi.InvokeOutputOptions{InvokeOptions: internal.PkgInvokeDefaultOpts(opts)}
			return ctx.InvokeOutput("akamai:index/getPropertyRulesBuilder:getPropertyRulesBuilder", args, GetPropertyRulesBuilderResultOutput{}, options).(GetPropertyRulesBuilderResultOutput), nil
		}).(GetPropertyRulesBuilderResultOutput)
}

// A collection of arguments for invoking getPropertyRulesBuilder.
type GetPropertyRulesBuilderOutputArgs struct {
	RulesV20230105 GetPropertyRulesBuilderRulesV20230105PtrInput `pulumi:"rulesV20230105"`
	RulesV20230530 GetPropertyRulesBuilderRulesV20230530PtrInput `pulumi:"rulesV20230530"`
	RulesV20230920 GetPropertyRulesBuilderRulesV20230920PtrInput `pulumi:"rulesV20230920"`
	RulesV20231030 GetPropertyRulesBuilderRulesV20231030PtrInput `pulumi:"rulesV20231030"`
	RulesV20240109 GetPropertyRulesBuilderRulesV20240109PtrInput `pulumi:"rulesV20240109"`
	RulesV20240212 GetPropertyRulesBuilderRulesV20240212PtrInput `pulumi:"rulesV20240212"`
	RulesV20240531 GetPropertyRulesBuilderRulesV20240531PtrInput `pulumi:"rulesV20240531"`
	RulesV20240813 GetPropertyRulesBuilderRulesV20240813PtrInput `pulumi:"rulesV20240813"`
	RulesV20241021 GetPropertyRulesBuilderRulesV20241021PtrInput `pulumi:"rulesV20241021"`
	RulesV20250113 GetPropertyRulesBuilderRulesV20250113PtrInput `pulumi:"rulesV20250113"`
	RulesV20250218 GetPropertyRulesBuilderRulesV20250218PtrInput `pulumi:"rulesV20250218"`
}

func (GetPropertyRulesBuilderOutputArgs) ElementType() reflect.Type {
	return reflect.TypeOf((*GetPropertyRulesBuilderArgs)(nil)).Elem()
}

// A collection of values returned by getPropertyRulesBuilder.
type GetPropertyRulesBuilderResultOutput struct{ *pulumi.OutputState }

func (GetPropertyRulesBuilderResultOutput) ElementType() reflect.Type {
	return reflect.TypeOf((*GetPropertyRulesBuilderResult)(nil)).Elem()
}

func (o GetPropertyRulesBuilderResultOutput) ToGetPropertyRulesBuilderResultOutput() GetPropertyRulesBuilderResultOutput {
	return o
}

func (o GetPropertyRulesBuilderResultOutput) ToGetPropertyRulesBuilderResultOutputWithContext(ctx context.Context) GetPropertyRulesBuilderResultOutput {
	return o
}

// The provider-assigned unique ID for this managed resource.
func (o GetPropertyRulesBuilderResultOutput) Id() pulumi.StringOutput {
	return o.ApplyT(func(v GetPropertyRulesBuilderResult) string { return v.Id }).(pulumi.StringOutput)
}

func (o GetPropertyRulesBuilderResultOutput) Json() pulumi.StringOutput {
	return o.ApplyT(func(v GetPropertyRulesBuilderResult) string { return v.Json }).(pulumi.StringOutput)
}

func (o GetPropertyRulesBuilderResultOutput) RuleFormat() pulumi.StringOutput {
	return o.ApplyT(func(v GetPropertyRulesBuilderResult) string { return v.RuleFormat }).(pulumi.StringOutput)
}

func (o GetPropertyRulesBuilderResultOutput) RulesV20230105() GetPropertyRulesBuilderRulesV20230105PtrOutput {
	return o.ApplyT(func(v GetPropertyRulesBuilderResult) *GetPropertyRulesBuilderRulesV20230105 { return v.RulesV20230105 }).(GetPropertyRulesBuilderRulesV20230105PtrOutput)
}

func (o GetPropertyRulesBuilderResultOutput) RulesV20230530() GetPropertyRulesBuilderRulesV20230530PtrOutput {
	return o.ApplyT(func(v GetPropertyRulesBuilderResult) *GetPropertyRulesBuilderRulesV20230530 { return v.RulesV20230530 }).(GetPropertyRulesBuilderRulesV20230530PtrOutput)
}

func (o GetPropertyRulesBuilderResultOutput) RulesV20230920() GetPropertyRulesBuilderRulesV20230920PtrOutput {
	return o.ApplyT(func(v GetPropertyRulesBuilderResult) *GetPropertyRulesBuilderRulesV20230920 { return v.RulesV20230920 }).(GetPropertyRulesBuilderRulesV20230920PtrOutput)
}

func (o GetPropertyRulesBuilderResultOutput) RulesV20231030() GetPropertyRulesBuilderRulesV20231030PtrOutput {
	return o.ApplyT(func(v GetPropertyRulesBuilderResult) *GetPropertyRulesBuilderRulesV20231030 { return v.RulesV20231030 }).(GetPropertyRulesBuilderRulesV20231030PtrOutput)
}

func (o GetPropertyRulesBuilderResultOutput) RulesV20240109() GetPropertyRulesBuilderRulesV20240109PtrOutput {
	return o.ApplyT(func(v GetPropertyRulesBuilderResult) *GetPropertyRulesBuilderRulesV20240109 { return v.RulesV20240109 }).(GetPropertyRulesBuilderRulesV20240109PtrOutput)
}

func (o GetPropertyRulesBuilderResultOutput) RulesV20240212() GetPropertyRulesBuilderRulesV20240212PtrOutput {
	return o.ApplyT(func(v GetPropertyRulesBuilderResult) *GetPropertyRulesBuilderRulesV20240212 { return v.RulesV20240212 }).(GetPropertyRulesBuilderRulesV20240212PtrOutput)
}

func (o GetPropertyRulesBuilderResultOutput) RulesV20240531() GetPropertyRulesBuilderRulesV20240531PtrOutput {
	return o.ApplyT(func(v GetPropertyRulesBuilderResult) *GetPropertyRulesBuilderRulesV20240531 { return v.RulesV20240531 }).(GetPropertyRulesBuilderRulesV20240531PtrOutput)
}

func (o GetPropertyRulesBuilderResultOutput) RulesV20240813() GetPropertyRulesBuilderRulesV20240813PtrOutput {
	return o.ApplyT(func(v GetPropertyRulesBuilderResult) *GetPropertyRulesBuilderRulesV20240813 { return v.RulesV20240813 }).(GetPropertyRulesBuilderRulesV20240813PtrOutput)
}

func (o GetPropertyRulesBuilderResultOutput) RulesV20241021() GetPropertyRulesBuilderRulesV20241021PtrOutput {
	return o.ApplyT(func(v GetPropertyRulesBuilderResult) *GetPropertyRulesBuilderRulesV20241021 { return v.RulesV20241021 }).(GetPropertyRulesBuilderRulesV20241021PtrOutput)
}

func (o GetPropertyRulesBuilderResultOutput) RulesV20250113() GetPropertyRulesBuilderRulesV20250113PtrOutput {
	return o.ApplyT(func(v GetPropertyRulesBuilderResult) *GetPropertyRulesBuilderRulesV20250113 { return v.RulesV20250113 }).(GetPropertyRulesBuilderRulesV20250113PtrOutput)
}

func (o GetPropertyRulesBuilderResultOutput) RulesV20250218() GetPropertyRulesBuilderRulesV20250218PtrOutput {
	return o.ApplyT(func(v GetPropertyRulesBuilderResult) *GetPropertyRulesBuilderRulesV20250218 { return v.RulesV20250218 }).(GetPropertyRulesBuilderRulesV20250218PtrOutput)
}

func init() {
	pulumi.RegisterOutputType(GetPropertyRulesBuilderResultOutput{})
}
